...
const DATA = raw.map(r => ({
  id: r.id, city: r.features.city, tier: (r.features.articleship_tier || 'Other'),
...
profileForm.onsubmit = (e) => {
  ...
  const profile = {
    city, tier: document.getElementById('tier').value, domain: document.getElementById('domain').value || null,
...
function createPivotControls(state, onUpdate) {
  ...
  // Example filter
  container.innerHTML += createSelect('filter-tier', 'Filter by Tier', { '': 'All Tiers', ...Object.fromEntries([...new Set(DATA.map(d=>d.tier))].sort().map(t => [t,t])) }, state.filters.tier || '');
...
function renderInteractivePivot(pivotData, container, state) {
  ...
  container.onclick = (e) => {
    if (e.target.tagName === 'BUTTON') {
      ...
      const filters = {...state.filters, [state.rowField]: rowVal, [state.colField]: colVal};
      ...
      renderDetailsTable(matchingData, e.target.closest('.tabular-section') || document.getElementById('tabular-results-container'));
    }
  };
}
...
function renderDetailsTable(data, container) {
    ...
    let tableHTML = `<h3>Top Matching Profiles (${data.length} found)</h3><div class="table-container"><table class="pivot-table"><thead><tr>
        <th>Shortlists</th><th>City</th><th>Tier</th><th>Domain</th><th>Score</th><th>Attempts</th><th>IT</th><th>Rank</th>
    </tr></thead><tbody>`;
    data.slice(0, 20).forEach(d => { // Show top 20 matches
        tableHTML += `<tr>
...
function trimmedMean(arr, percent) {
  ...
  if (!trimmedArr.length) return arr.reduce((a, b) => a + b, 0) / arr.length; // Fallback for small arrays
...
function buildCityCompanyPrior(data){
  ...
  data.forEach(d => {
    ...
    (d.shortlisted_by||[]).forEach(c => byCity.get(city).set(c, (byCity.get(city).get(c)||0)+1));
  });
