...
document.addEventListener('click', (event) => {
  if (!expandedMenu.contains(event.target) && !menuButton.contains(event.target) && expandedMenu.classList.contains('active')) {
...
function getApplicationLink(applicationId) {
  if (isValidUrl(applicationId)) return applicationId;
  let emails = applicationId.split(/,|\s/).filter(e => e);
  ...
  return `mailto:${firstEmail}?subject=${subject}`;
}
...
function populateSalaryFilter(table) {
  ...
  if(table==="Articleship Jobs") {
    ...
    filters = [
      ...
      { value:'20000-40000', text:'₹20,000 - ₹40,000' },
    ...
    filters = [
      ...
      { value:'0-25000', text:'Below ₹25,000' },
      { value:'25000-35000', text:'₹25,000 - ₹35,000' },
      { value:'35000-50000', text:'₹25,000 - ₹50,000' },
      { value:'50000+', text:'Above ₹50,000' }
    ];
  }
  else if(table==="Fresher Jobs") {
    filters = [
      { value:'', text:'Any Salary' },
      { value:'0-12', text:'Less than 12 LPA' },
      { value:'12-18', text:'12-18 LPA' },
      { value:'18+', text:'More than 18 LPA' }
    ];
  }
  filters.forEach(f => {
...
async function fetchJobs(searchTerm='', locationSearch='', salary='', category='') {
  ...
  try {
    ...
    if(locationSearch){
      let pattern = `%${locationSearch}%`;
      query = query.ilike('Location', pattern);
    }
    if(salary){
      if(salary.includes('+')){
        let min = Number(salary.replace('+',''));
        query = query.gte('Salary', min);
      } else if(salary.includes('-')){
        let [min, max] = salary.split('-').map(Number);
...
function handleScroll(){
  ...
  if(currentScrollY>lastScrollY && currentScrollY>100) header.classList.add('header-hidden');
  else header.classList.remove('header-hidden');
  lastScrollY = currentScrollY;
}
function highlightSearchTerm(text, searchTerm){
  if(!searchTerm || !text) return text;
  let regex = new RegExp(`(${searchTerm.split(/\s+/).join('|')})`, 'gi');
  return text.replace(regex, '<span class="highlight">$&</span>');
}
async function loadBanners(){
  try{
    const { data: banners, error } = await supabaseClient.from('Banners').select('Image, Hyperlink');
    if(error) return;
...
export function updateHeaderAuth(session){
  ...
  if(session){
    ...
    let userInitial = userEmail.charAt(0).toUpperCase();
    authButtons.innerHTML = `
      <div class="user-profile-container">
        <div class="user-icon-wrapper">
          <div class="user-icon" data-email="${userEmail}">${userInitial}</div>
    ...
    authButtons.innerHTML = `<a href="/login" class="auth-icon-btn"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></a>`;
  }
}
...
searchInput.addEventListener('input', (e)=>{
  ...
  timeout = setTimeout(()=>{
    ...
    hasMoreData = true;
    loadMoreButton.style.display = 'none';
    fetchJobs(e.target.value, locationSearchInput.value, salaryFilter.value, categoryFilter.value);
  },300);
});
locationSearchInput.addEventListener('input', (e)=>{
  ...
  timeout = setTimeout(()=>{
    ...
    hasMoreData = true;
    loadMoreButton.style.display = 'none';
    fetchJobs(searchInput.value, e.target.value, salaryFilter.value, categoryFilter.value);
  },300);
});
...
async function fetchCategories(){
  try{
    const { data, error } = await supabaseClient.from(currentTable).select('Category');
    if(error) return;
