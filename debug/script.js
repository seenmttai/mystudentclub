...
function showModal(job) {
    const companyInitial = job.Company ? job.Company.charAt(0).toUpperCase() : '?';
    const postedDate = job.Created_At ? getDaysAgo(job.Created_At) : 'N/A';
    const linkedinSearchTerm = `Industrial Trainee at ${job.Company}`;
    const linkedinUrl = `https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(linkedinSearchTerm)}`;
    ...
    modalBody.innerHTML = `
        ...
        <div class="modal-actions">
            ...
            <a href="${linkedinUrl}" class="btn btn-secondary" target="_blank">Connect to Peer</a>
        </div>
    `;
