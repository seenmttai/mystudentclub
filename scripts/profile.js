...
const ENTRY_CLEAR_MAP = {
  ...
  "edu-final-display": [
    "ca_final_course",
    "ca_final_attempts_type",
    "ca_final_attempts",
    "ca_final_clear_month",
    "ca_final_clear_year",
    "ca_final_app_month",
    "ca_final_app_year",
  ],
  "edu-inter-display": [
    "ca_inter_course",
    "ca_inter_attempts_type",
    "ca_inter_attempts",
    "ca_inter_clear_month",
    "ca_inter_clear_year",
  ],
  "edu-found-display": [
    "ca_found_course",
    "ca_found_attempts_type",
    "ca_found_attempts",
    "ca_found_clear_month",
    "ca_found_clear_year",
  ],
  "edu-grad-display": [
    "grad_degree",
  ...
  "emp-art-display": [
    "articleship_firm_type",
    "articleship_firm_name",
    "articleship_domain",
    "articleship_domain_other",
  ],
  ...
  "project-entry-display": [
    ...
    "project_client",
    "project_status",
    "project_worked_from_year",
    "project_worked_from_month",
    "project_details",
...
function populateForm(profileData) {
  ...
  const domainOtherGroup = document.getElementById(
    "articleship_domain_other_group",
  );
...
function attachEntryRemoveButtons() {
  document
    .querySelectorAll(".p2-saved-entry .p2-entry-edit[data-toggle]")
    .forEach((editLink) => {
...
async function handleFile(file, type) {
  ...
  if (type === "resume" && file.size > 5 * 1024 * 1024) {
    showToast(
      "Resume file size exceeds the 5 MB limit. Please upload a smaller file.",
      "error",
    );
  ...
  try {
    ...
    if (type === "cover_letter" && file.type !== "application/pdf") {
      showToast(
        "Please upload your Cover Letter in PDF format only.",
        "warning",
      );
      return;
    }
        ...
        if (images.length > 0) {
          try {
            localStorage.setItem(
              config.storageKeyImages,
              JSON.stringify(images),
            );
          } catch (e) {
            showToast(
              "Resume is too large. Please reupload your resume.",
              "error",
            );
          }
          ...
          if (extractResult && extractResult.is_valid === false) {
            ...
            refreshHeader();
            showToast(
              "Profile auto-filled from your resume! Please review the details.",
              "success",
              8000,
            );
          }
    ...
    console.error(error);
    showToast(
      `Could not process your ${type.replace("_", " ")}. Please try a different file.`,
      "error",
    );
  } finally {
    showLoading(false);
  ...
  try {
    ...
    if (data.ok && data.response) {
      ...
      if (parsed.is_valid_cv === false) {
        return {
          is_valid: false,
          message: "The uploaded file does not appear to be a valid Resume/CV.",
        };
      }
...
async function loadConsentStatus() {
  ...
  try {
    ...
    if (data && data.cv_sharing_consent) {
      ...
      if (privacyStatus) {
        privacyStatus.textContent = "Active";
        privacyStatus.style.color = "#16A34A";
      }
      ...
      if (privacyStatus) {
        privacyStatus.textContent = "Not provided";
        privacyStatus.style.color = "";
      }
...
function updatePrivacyCard(isConsented) {
  ...
  if (isConsented) {
    ...
    if (privacyDate)
      privacyDate.textContent = new Date().toLocaleDateString("en-IN", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      });
...
async function handleSave(e) {
  ...
  if (
    ...
    const nameVal = displayNameInput
      ? (displayNameInput.value || displayNameInput.textContent || "")
          .trim()
          .toLowerCase()
      : "";
    ...
    const phoneVal = phoneInput
      ? (phoneInput.value || "").trim().toLowerCase()
      : "";

    const textLower = ocrText.toLowerCase();

    const nameMatch =
      nameVal && nameVal !== "your name" && textLower.includes(nameVal);
    ...
    if (true || nameMatch || emailMatch || phoneMatch) {
      ...
      try {
        ...
        const syncResponse = await fetch(
          "https://storer.bhansalimanan55.workers.dev",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (syncResponse.ok) {
          ...
          if (syncData.ok && syncData.response) {
            ...
            if (syncData.uploaded) {
              ...
              console.error(
                "Cloud storage sync failed:",
                syncData.storage_error,
              );
              showToast(
                "Resume saved, but cloud backup failed. It will retry on next save.",
                "warning",
                8000,
              );
            }
        ...
        showToast(
          "Network error while backup syncing resume.",
          "warning",
          8000,
        );
      }
  ...
  if (profileData.job_preference) {
    ...
    if (prev !== profileData.job_preference)
  ...
  try {
    ...
    console.error(e);
    showToast(
      "Profile saved to your browser, but failed to sync to the server. You can continue using the site.",
      "warning",
      10000,
    );
  } finally {
...
function refreshHeader() {
  ...
  // Header details
  document.getElementById("hdLocation").textContent =
    d.current_location || "Add location";
  document.getElementById("hdPhone").textContent =
    d.contact_number || "Add mobile number";
  document.getElementById("hdExperience").textContent =
    d.total_experience || "Fresher";
  document.getElementById("hdEmail").textContent =
    d.email || (currentUser ? currentUser.email : "Add email");
  document.getElementById("hdNotice").textContent =
    d.notice_period || "Add availability to join";
  ...
  if (hdLinkedinItem && hdLinkedinLink) {
    ...
    if (linkedinRaw) {
      const linkedinUrl = /^[a-zA-Z][a-zA-Z\d+\-.]*:\/\//.test(linkedinRaw)
        ? linkedinRaw
        : `https://${linkedinRaw}`;
  ...
  if (hdSummaryWrap && hdSummaryText) {
    if (summaryVal) {
      ...
      hdSummaryText.textContent =
        summaryVal.length > 260 ? `${summaryVal.slice(0, 260)}...` : summaryVal;
    } else {
  ...
  const hasExperience = !!(
    ...
    ((d.articleship_firm_type || "").trim() &&
      (d.articleship_firm_type || "").trim() !== "None") ||
  ...
  const items = [
    { label: "Add full name", icon: "fa-user", filled: !!nameVal, boost: 10 },
    {
      ...
      icon: "fa-phone-alt",
      filled: !!(d.contact_number || "").trim(),
      boost: 10,
    },
    {
      label: "Add location",
      icon: "fa-map-marker-alt",
      filled: !!(d.current_location || "").trim(),
      boost: 2,
    },
    {
      label: "Add resume",
      icon: "fa-file-alt",
      filled: !!localStorage.getItem("userCVText"),
      boost: 10,
    },
    {
      label: "Add profile summary",
      icon: "fa-heading",
      filled: !!(d.profile_summary || d.headline || "").trim(),
      boost: 8,
    },
    {
      label: "Add CA education",
    ...
    },
    {
      ...
      icon: "fa-briefcase",
      filled: hasExperience,
      boost: 10,
    },
    {
      label: "Add notice period",
      icon: "fa-calendar-check",
      filled: !!(d.notice_period || "").trim(),
      boost: 5,
    },
    {
      label: "Add current organization",
      icon: "fa-building",
      filled: hasCurrentOrg,
      boost: 5,
    },
    {
  ...
  if (needsCTC) {
    items.push({
      label: "Add current CTC",
      icon: "fa-wallet",
      filled: !!(d.current_ctc || "").trim(),
      boost: 3,
    });
  }

  const totalBoost = items.reduce((s, i) => s + i.boost, 0);
  const filledBoost = items
    .filter((i) => i.filled)
    .reduce((s, i) => s + i.boost, 0);
  ...
  document.querySelectorAll(".p2-ql").forEach((link) => {
    ...
    let filled = false;
    if (hrefTarget === "#sec-resume")
      filled = !!localStorage.getItem("userCVText");
    else if (hrefTarget === "#sec-headline")
      filled = !!(d.profile_summary || d.headline || "").trim();
    else if (hrefTarget === "#sec-ca-education")
      filled = !!(
        (d.ca_final_course || "") +
        (d.ca_inter_course || "") +
        (d.ca_found_course || "") +
    ...
    else if (hrefTarget === "#sec-experience")
      filled = !!(
        (d.total_experience || "").trim() ||
        (d.emp_company_name || "").trim() ||
        (d.emp_job_title || "").trim() ||
    ...
    else if (hrefTarget === "#sec-projects")
      filled = !!(d.project_title || "").trim();
    else if (hrefTarget === "#sec-career")
      filled = !!(
        (d.job_preference || "") +
        (d.current_industry || "") +
        (d.department || "") +
        ...
        (d.job_role || "") +
        (d.desired_job_type || "") +
        (d.desired_employment_type || "") +
        (d.preferred_shift || "") +
        (d.preferred_locations || "") +
        (d.expected_salary || "")
      ).trim();
    else if (hrefTarget === "#sec-certification")
      filled = !!(
        (d.cert_name || "") +
        (d.cert_issuer || "") +
        (d.cert_year || "")
...
function refreshSavedDisplays(d) {
  ...
  const personalParts = [gender && gender.toLowerCase(), marital].filter(
    Boolean,
  );
  ...
  if (langBody) {
    ...
    if (!rows.length) {
      langBody.innerHTML =
        '<tr><td colspan="5" class="p2-lang-empty">Add languages</td></tr>';
    } else {
      langBody.innerHTML = rows
        .map(
          (r) => `
                <tr>
                    <td>${r.language}</td>
  ...
  if (summaryVal) {
    ...
    if (summaryEditToggle)
      summaryEditToggle.textContent = "Edit profile summary";
  } else {
    if (summaryEntry) summaryEntry.style.display = "none";
    if (summaryBoost) summaryBoost.style.display = "inline-block";
    if (summaryEditToggle)
      summaryEditToggle.textContent = "Add profile summary";
  }
  ...
  if (finalCourse) {
    ...
    if (["articleship", "industrial"].includes(pref)) {
      ...
      if (appMonth && appYear)
        metaParts.push(`Expected: ${appMonth} ${appYear}`);
    } else {
      ...
      if (clearMonth && clearYear)
        metaParts.push(`Cleared: ${clearMonth} ${clearYear}`);
    }
    document.getElementById("edu-final-meta").textContent =
      metaParts.join(" · ");
  ...
  if (interCourse) {
    ...
    if (clearMonth && clearYear)
      metaParts.push(`Cleared: ${clearMonth} ${clearYear}`);
    document.getElementById("edu-inter-meta").textContent =
      metaParts.join(" · ");
  ...
  if (foundCourse) {
    ...
    if (clearMonth && clearYear)
      metaParts.push(`Cleared: ${clearMonth} ${clearYear}`);
    document.getElementById("edu-found-meta").textContent =
      metaParts.join(" · ");
  ...
  if (gradDegree) {
    ...
    let metaParts = [];
    if ((d.grad_university || "").trim())
      metaParts.push(d.grad_university.trim());
    if ((d.grad_year || "").trim()) metaParts.push(d.grad_year);
    if ((d.grad_percentage || "").trim())
      metaParts.push(d.grad_percentage.trim());
    document.getElementById("edu-grad-meta").textContent =
      metaParts.join(" · ");
  ...
  if (c12Board) {
    ...
    document.getElementById("edu-12-title").textContent =
      `Class XII — ${c12Board}`;
    let metaParts = [];
    if ((d.class12_school || "").trim())
      metaParts.push(d.class12_school.trim());
    if ((d.class12_year || "").trim()) metaParts.push(d.class12_year);
    if ((d.class12_percentage || "").trim())
      metaParts.push(d.class12_percentage.trim());
  ...
  if (c10Board) {
    ...
    document.getElementById("edu-10-title").textContent =
      `Class X — ${c10Board}`;
    let metaParts = [];
    if ((d.class10_school || "").trim())
      metaParts.push(d.class10_school.trim());
    if ((d.class10_year || "").trim()) metaParts.push(d.class10_year);
    if ((d.class10_percentage || "").trim())
      metaParts.push(d.class10_percentage.trim());
  ...
  if (otherEduCourse || otherEduLevel) {
    ...
    document.getElementById("edu-other-title").textContent =
      otherEduCourse || otherEduLevel || "Other Education";
    let metaParts = [];
    if (otherEduLevel && otherEduCourse) metaParts.push(otherEduLevel);
    if ((d.other_edu_institute || "").trim())
      metaParts.push(d.other_edu_institute.trim());
    if ((d.other_edu_year || "").trim()) metaParts.push(d.other_edu_year);
    if ((d.other_edu_score || "").trim())
      metaParts.push(d.other_edu_score.trim());
    document.getElementById("edu-other-meta").textContent =
      metaParts.join(" | ");
  ...
  if (eduEditToggle) {
    eduEditToggle.textContent = hasEducation
      ? "Edit education"
      : "Add education";
  }
  ...
  if (currentCompany || currentTitle) {
    ...
    document.getElementById("emp-org-title").textContent =
  ...
  if (artFirm || (artType && artType !== "None")) {
    ...
    document.getElementById("emp-art-title").textContent =
      artFirm || "Articleship";
    let metaParts = [];
    if (artType && artType !== "None") metaParts.push(artType);
    if ((d.articleship_domain || "").trim())
      metaParts.push(d.articleship_domain);
  ...
  if (itCompany) {
    ...
    document.getElementById("emp-it-meta").textContent =
      "MSC Industrial Training Program";
  ...
  if (projectTitle) {
    ...
    if ((d.project_client || "").trim())
      metaParts.push(`Client: ${d.project_client.trim()}`);
    if ((d.project_status || "").trim())
      metaParts.push(d.project_status.trim());
    const workedFrom = [
      (d.project_worked_from_month || "").trim(),
      (d.project_worked_from_year || "").trim(),
    ]
      .filter(Boolean)
      .join(" ");
    if (workedFrom) metaParts.push(`Worked from: ${workedFrom}`);
    const projectMetaDisplay = document.getElementById("project-meta-display");
    if (projectMetaDisplay)
      projectMetaDisplay.textContent = metaParts.join(" | ");

    const projectDescDisplay = document.getElementById("project-desc-display");
    if (projectDescDisplay)
      projectDescDisplay.textContent = (d.project_details || "").trim();
    const attachmentName = (d.project_attachment_name || "").trim();
    const projectAttachmentDisplay = document.getElementById(
      "project-attachment-display",
    );
    if (projectAttachmentDisplay)
      projectAttachmentDisplay.textContent = attachmentName
  ...
  setCareerValue(
    "career-desired-job-type",
    d.desired_job_type,
    "Add desired job type",
  );
  setCareerValue(
    "career-desired-employment-type",
    d.desired_employment_type,
    "Add desired employment type",
  );
  setCareerValue(
    "career-preferred-shift",
    d.preferred_shift,
    "Add preferred shift",
  );
  setCareerValue(
    "career-preferred-location",
    d.preferred_locations,
    "Add preferred location",
  );
  setCareerValue(
    "career-expected-salary",
    d.expected_salary,
    "Add expected salary",
  );
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const artCompGroup = document.getElementById(
    "articleship_completion_date_group",
  );
  ...
  if (jobPrefSelect)
    jobPrefSelect.addEventListener("change", handleJobPrefChange);
  ...
  if (domainSelect && domainOtherGroup) {
    domainSelect.addEventListener("change", () => {
      domainOtherGroup.style.display =
        domainSelect.value === "Other" ? "block" : "none";
    });
  }
  ...
  function serializeLanguages(rows) {
    return rows
      ...
      .map(
        (r) =>
          `${r.language.trim()}|${r.proficiency || "Beginner"}|${r.read ? "1" : "0"}|${r.write ? "1" : "0"}|${r.speak ? "1" : "0"}`,
      )
      .join(", ");
  }
  ...
  function renderLanguageRows() {
    ...
    langEditorRows.innerHTML = personalLanguages
      .map(
        (r, idx) => `
  ...
  function loadLanguageRowsFromHidden() {
    ...
    personalLanguages = parsed.length
      ? parsed
      : [
          {
            language: "",
  ...
  if (langEditorRows) {
    langEditorRows.addEventListener("input", (e) => {
      ...
      personalLanguages[idx].language =
        row.querySelector(".lang-name")?.value || "";
      personalLanguages[idx].proficiency =
        row.querySelector(".lang-prof")?.value || "Beginner";
      personalLanguages[idx].read = !!row.querySelector(".lang-read")?.checked;
      personalLanguages[idx].write =
        !!row.querySelector(".lang-write")?.checked;
      personalLanguages[idx].speak =
        !!row.querySelector(".lang-speak")?.checked;
      if (languagesHiddenInput)
        languagesHiddenInput.value = serializeLanguages(personalLanguages);
    });
  ...
  ["resume", "cover_letter"].forEach((type) => {
    ...
    if (config.dropZone) {
      ["dragenter", "dragover", "dragleave", "drop"].forEach((ev) => {
        config.dropZone.addEventListener(
          ev,
          (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (["dragenter", "dragover"].includes(ev))
  ...
  function updateSummaryCharCount() {
    ...
    summaryLeft.textContent = `${Math.max(0, left)}`;
    if (headlineHidden)
      headlineHidden.value = (profileSummaryInput.value || "").trim();
  }
    ...
    document.querySelectorAll(".p2-card-form").forEach((form) => {
      ...
      if (!form.querySelector(".p2-sheet-header")) {
        ...
        if (existingTitle) {
          ...
          const cardH2 = form
            .closest(".p2-card")
            ?.querySelector(".p2-card-header h2");
      ...
      form
        .querySelectorAll(
          ".p2-modal-close:not(.p2-sheet-header .p2-modal-close)",
        )
        .forEach((b) => b.remove());
      ...
      if (
        !actions.querySelector(".p2-inline-save[data-save]") &&
        !actions.querySelector(".p2-btn-save")
      ) {
        ...
        actions.querySelector(".p2-btn-save") &&
        !actions.querySelector(".p2-btn-save[data-save]")
      ) {
        actions
          .querySelector(".p2-btn-save")
          .setAttribute("data-save", form.id);
      }
    });
  }
  ...
  function findFirstInvalidControl() {
    ...
    const controls = Array.from(profileForm.elements).filter(
      (el) =>
        el &&
  ...
  document
    .querySelectorAll(".p2-display-clickable[data-toggle]")
    .forEach((display) => {
  ...
  document.body.addEventListener("click", (e) => {
    const saveActionBtn = e.target.closest(
      ".p2-inline-save[data-save], .p2-btn-save[data-save]",
    );
  ...
  if (backdrop) {
    backdrop.addEventListener("click", () => {
      document
        .querySelectorAll(".p2-card-form:not(.collapsed)")
        .forEach((f) => {
  ...
  const authButtonsContainer = document.querySelector(
  ...
  if (user && authButtonsContainer) {
    ...
    const userIconWrapper =
      authButtonsContainer.querySelector(".user-icon-wrapper");
    const userHoverCard =
      authButtonsContainer.querySelector(".user-hover-card");
  ...
  const consentModalAcceptBtn = document.getElementById(
    "consentModalAcceptBtn",
  );
    ...
    cvConsentCheckbox.addEventListener("change", async () => {
      if (cvConsentCheckbox.checked) {
        ...
        if (saved) {
          await loadConsentStatus();
          showToast(
            "CV sharing consent saved. Your CV can now be shared with employers.",
            "success",
            6000,
          );
        } else {
        ...
        if (saved) {
          ...
          showToast(
            "CV sharing consent withdrawn. Your CV will no longer be shared with employers.",
            "warning",
            8000,
          );
        } else {
