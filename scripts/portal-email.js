...
function setActivePortalTab() {
    // For single page email portal, we are fetching all tables. Do nothing here.
}
...
function shareJob(job, btnElement) {
    ...
    const jobType = table === 'Industrial Training Job Portal' ? 'industrial' :
        table === 'Fresher Jobs' ? 'fresher' :
            table === 'Semi Qualified Jobs' ? 'semi' : 'articleship';
    ...
    try {
        const { error } = await supabaseClient
            ...
            .insert({
                ...
                job_id: job.id,
                job_table: job.source_table || state.portalType,
...
function showModal(job) {
    ...
    if (!connectLink) {
        ...
        } else if (table === 'Fresher Jobs' || table === 'Semi Qualified Jobs') {
    ...
    try {
        if (page === 0 || !cachedMergedJobs) {
            ...
            const queries = tablesToFetch.map(async (table) => {
                ...
                if (table === "Fresher Jobs") selectColumns = selectColumnsFresher;
                else if (table === "Semi Qualified Jobs") selectColumns = selectColumnsSemi;

                let query = supabaseClient.from(table).select(selectColumns);

                // ONLY EMAILS
                query = query.ilike('Application ID', '%@%').not('Application ID', 'ilike', 'http%');

                // Apply Keyword filters
                if (state.keywords && state.keywords.length > 0) {
                    const keywordOrs = state.keywords.map(kw => {
                        const flexibleTerm = kw.trim().replace(/\s+/g, '%');
                        return `Company.ilike."%${flexibleTerm}%",Description.ilike."%${flexibleTerm}%",Category.ilike."%${flexibleTerm}%",Location.ilike."%${flexibleTerm}%"`;
                    });
                    query = query.or(keywordOrs.join(','));
                }

                if (state.locations && state.locations.length > 0) {
                    const locationOr = state.locations.map(loc => `Location.ilike."%${loc}%"`);
                    query = query.or(locationOr.join(','));
                }

                if (state.categories && state.categories.length > 0) {
                    const categoryOr = state.categories.map(cat => `Category.ilike."%${cat}%"`);
                    query = query.or(categoryOr.join(','));
                }

                if (state.applicationStatus === 'not_applied' && currentSession && appliedJobIds.size > 0) {
                    const tupleString = `(${Array.from(appliedJobIds).join(',')})`;
                    query = query.not('id', 'in', tupleString);
                }

                query = query.order('Created_At', { ascending: false, nullsFirst: false }).limit(300);
            ...
            cachedMergedJobs.sort((a, b) => {
                ...
                if (sortCol === 'Salary') {
                    const s1 = parseInt(a.Salary) || 0;
                    const s2 = parseInt(b.Salary) || 0;
                    return isAsc ? s1 - s2 : s2 - s1;
                }
        ...
        const pagedData = cachedMergedJobs.slice(start, start + limit);

        if (pagedData && pagedData.length > 0) {
            const fragment = document.createDocumentFragment();
            for (let i = 0; i < pagedData.length; i++) {
                fragment.appendChild(renderJobCard(pagedData[i]));
            }
            if (dom.jobsContainer) {
                dom.jobsContainer.appendChild(fragment);
            }
            page++;
            hasMoreData = start + limit < cachedMergedJobs.length;
        } else {
            hasMoreData = false;
        ...
        if (typeof requestAnimationFrame !== 'undefined') requestAnimationFrame(updateUI);
        else setTimeout(updateUI, 0);
    }
}
...
async function markJobAsApplied(job) {
    ...
    let dbJobId = job.id;
    let dbJobTable = job.source_table || state.portalType;
...
function constructMailto(job, body = "") {
    ...
    const table = job.source_table || state.portalType;
    const subjectBase = EMAIL_SUBJECT_MAP[table] || `Application for the role`;
...
async function loadBanners() {
    ...
    try {
        ...
        const banners = data;
        carousel.innerHTML = '';
        const table = state.portalType === 'all' ? 'Industrial Training Job Portal' : state.portalType;
        const currentType = table === "Semi Qualified Jobs" ? "Semi-Qualified" : table === "Fresher Jobs" ? "Freshers" : table.split(' ')[0];
...
function setupEventListeners() {
    ...
    const handlePortalChange = (e) => {
        ...
        if (state.portalType !== newPortal) {
            updateState({ portalType: newPortal });
            if (e.target.id === 'portalFilterDesktop' && dom.portalFilterMobile) {
                dom.portalFilterMobile.value = newPortal;
            } else if (e.target.id === 'portalFilterMobile' && dom.portalFilterDesktop) {
                dom.portalFilterDesktop.value = newPortal;
            }
            syncAndFetch();
        }
    };
    if (dom.portalFilterDesktop) dom.portalFilterDesktop.addEventListener('change', handlePortalChange);
    if (dom.portalFilterMobile) dom.portalFilterMobile.addEventListener('change', handlePortalChange);
    ...
    [dom.desktopResetBtn, dom.mobileResetBtn].forEach(btn => {
        if (btn) btn.addEventListener('click', () => {
            ...
            state.sortBy = 'newest';
            state.applicationStatus = 'all';
            state.portalType = 'all';
            if (dom.portalFilterDesktop) {
                dom.portalFilterDesktop.value = 'all';
                dom.portalFilterDesktop.dispatchEvent(new Event('change'));
            }
            if (dom.portalFilterMobile) {
                dom.portalFilterMobile.value = 'all';
                dom.portalFilterMobile.dispatchEvent(new Event('change'));
            }
            if (dom.searchInputMobile) dom.searchInputMobile.value = '';
            if (state.portalType === 'Fresher Jobs') {
    ...
    document.querySelectorAll('.experience-filter-group .pill-options').forEach(group => {
        group.addEventListener('click', (e) => {
            if (e.target.classList.contains('pill-btn')) {
                if (state.portalType !== 'Semi Qualified Jobs') return;
...
function initCustomSelects() {
    const selectorIds = ['sortBySelect', 'salaryFilterDesktop', 'sortBySelectMobile', 'salaryFilterMobile', 'locationSelect', 'jobTypeSelect', 'portalFilterDesktop', 'portalFilterMobile'];
...
async function initializePage() {
    ...
    dom.portalFilterDesktop = document.getElementById('portalFilterDesktop');
    dom.portalFilterMobile = document.getElementById('portalFilterMobile');
...
function checkAndOpenSharedJob() {
    ...
    if (jobId && jobType) {
        if (jobType === 'semi') state.portalType = 'Semi Qualified Jobs';
        else if (jobType === 'fresher') state.portalType = 'Fresher Jobs';
        else if (jobType === 'articleship') state.portalType = 'Articleship Jobs';
        else state.portalType = 'Industrial Training Job Portal';

        if (dom.portalFilterDesktop) dom.portalFilterDesktop.value = state.portalType;
        if (dom.portalFilterMobile) dom.portalFilterMobile.value = state.portalType;

        // Fetch and open the specific job
        fetchSharedJob(jobId, state.portalType);
...
async function fetchSharedJob(jobId, table) {
    try {
        const { data, error } = await supabaseClient
            .from(table)
            .select('*')
...
document.addEventListener('DOMContentLoaded', async () => {
    setTimeout(async () => {
        ...
        if (currentSession && !preference) {
            // For unified email portal, we might not need this prompt
            // Let's just bypass it for now as this is specific to `jobs-by-email.html`
        } else if (currentSession && preference) {
            // Skip experience logic because it's handled by portals
