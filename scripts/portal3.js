...
async function fetchAllDistinct(column, tableName) {
...
async function fetchFilterOptions() {
    try {
        const [locations, categories] = await Promise.all([
            fetchAllDistinct('Location', currentTable),
            fetchAllDistinct('Category', currentTable)
        ]);
...
function setupEventListeners() {
    ...
    const searchInputs = [dom.searchInput, dom.searchInputDesktop].filter(Boolean);
