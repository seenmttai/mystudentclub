...
(function () {
  ...
  function open(opts = {}) {
    ...
    const wantsName = (opts.firmName || '').trim();
    const wantsLocation = (opts.location || '').trim();

    // Check if the saved draft matches the requested firm (if any specific firm requested)
    const firmMatches = draft && (
      wantsFirm
        ? (draft.firmId === wantsFirm || (wantsName && normName(draft.firmName) === normName(wantsName)))
        : wantsName
          ? normName(draft.firmName) === normName(wantsName)
          : true
    );
    ...
    if (firmMatches) {
      state = {
        step: draft.step || 1,
        firmId: wantsFirm || draft.firmId || null,
        firmName: wantsName || draft.firmName || '',
        location: wantsLocation || draft.location || (draft.savedData && draft.savedData.location) || '',
        lock: !!opts.lock,
      ...
      if (wantsName) state.savedData.firmName = wantsName;
      if (wantsLocation) state.savedData.location = wantsLocation;
    } else {
      state = {
        step: 1,
        firmId: wantsFirm,
  ...
  function renderStep1() {
      ...
      <form id="arw-step1">
        <div class="arw-grid-2">
          ...
          <div class="arw-field arw-location-wrap">
            ...
            <input id="arw-location" autocomplete="off" placeholder="e.g., Mumbai" value="${esc(state.location || '')}">
    ...
    // Pre-populate Step 1 if returning/resuming or passed via parameters
    const saved = state.savedData || {};
    if (state.firmName) {
      firmInput.value = state.firmName;
    } else if (saved.firmName) {
      firmInput.value = saved.firmName;
    }
    if (state.location) {
